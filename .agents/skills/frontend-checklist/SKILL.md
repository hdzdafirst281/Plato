...
You can read the detailed individual rules offline in the `references/rules/` directory (e.g., `references/rules/accessibility-testing.md`).
When auditing, use `list_dir` on `references/rules/` or search through them to apply specific checks if MCP is unavailable.
